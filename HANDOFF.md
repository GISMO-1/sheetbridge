# HANDOFF
Status: Background sync scheduler remains available and disabled by default until credentials arrive. FastAPI startup now invokes `sheetbridge.config.reload_settings()` so each lifespan reload picks up env overrides before touching the cache or schema and rebuilds the SQLite engine via `sheetbridge.store.refresh_engine()` before migrations run. `/append` retains idempotent writes backed by SQLite with configurable TTL + purge endpoint, enforces schema contracts with coercion + required-field checks, and now performs deterministic cache upserts whenever `KEY_COLUMN` is configured so duplicate keys refresh existing rows instead of creating new ones. Contract violations or missing keys (when `UPSERT_STRICT` is enabled) return HTTP 422 and are persisted to the dead-letter queue. `/bulk/append` extends those semantics to batch ingestion, returning per-row success/failure alongside DLQ writes, respecting an optional batch idempotency key, honoring `BULK_MAX_ITEMS`, and chunking optional Google Sheets write-back via `SHEETS_BATCH_SIZE` when credentials permit. `/rows` continues to offer substring filtering, column projection, and a `since` cursor based on cache insertion time alongside pagination metadata. Structured INFO logs now emit per request with method/path/status/latency, `/metrics` serves Prometheus counters/histograms for request totals and latency, and an optional per-IP token bucket limiter can gate traffic. Admin maintenance endpoints require either the legacy bearer token or a configured API key via the shared `require_auth` helper, and CORS middleware respects the `CORS_ALLOW_ORIGINS` allow-list. `/admin/schema`, `/admin/dlq`, and the new `/admin/dupes` endpoint expose schema state, the dead-letter queue, and duplicate key diagnostics over authenticated calls. Failed Google Sheets writes now land in the DLQ with reason `write_failed`, and the background retry worker now offloads blocking Google Sheets + SQLite work to a thread pool with bounded concurrency (controlled by `DLQ_RETRY_ENABLED`, `DLQ_RETRY_INTERVAL`, `DLQ_RETRY_BATCH`, and `DLQ_RETRY_CONCURRENCY`) so API handlers stay responsive; admins can still trigger manual drains via `POST /admin/dlq/retry`. OpenAPI is now locked to `openapi.json`; regenerate via `python -m sheetbridge.openapi_tool --out openapi.json` (or `make schema`) and CI enforces drift with a deterministic spec generator. Release automation now reads the package version from `sheetbridge.__version__`, publishes tagged Docker images to GHCR, and generates GitHub Releases with the changelog bundle when pushing `v0.x.y` tags. Production containers now boot through `docker/entrypoint.sh`, which wraps the app in Gunicorn with Uvicorn workers, honors `PORT`/`WORKERS`, and defaults to INFO logging while exposing a `/health` HTTP health check to Docker. A Compose bundle at the repo root wires the container, persists SQLite at `./data/sheetbridge.db`, and keeps OAuth tokens under `./tokens/sheets.json` for safe restarts while `.gitignore` shields the directory from commits; `.env.sample` documents the minimal knobs for quick runs.
...
10. Configure the new DLQ retry knobs (`DLQ_RETRY_ENABLED`, `DLQ_RETRY_INTERVAL`, `DLQ_RETRY_BATCH`) alongside Sheets write credentials, and exercise `POST /admin/dlq/retry` once production access is ready.
11. Monitor DLQ throughput under load and adjust `DLQ_RETRY_CONCURRENCY` to balance retry speed against Sheets quota + host CPU usage.
12. When ready to publish a new release, update `CHANGELOG.md`, tag the commit (`git tag v0.x.y && git push --tags`), verify the GHCR image (`ghcr.io/<owner>/sheetbridge:v0.x.y` and `:latest`), and share rollback guidance (pin consumers to a stable tag).
 13. For containerized deployments, tune `WORKERS`, adjust Compose volume mounts if relocating state, and pin production rollouts to a tagged GHCR image.
...
- Logging middleware: `sheetbridge/logging_setup.py` (INFO request logs) and legacy request-ID helper in `sheetbridge/logging.py`.
- Metrics endpoint wiring: `sheetbridge/metrics.py`
- Rate limiter: `sheetbridge/ratelimit.py`
- Scheduler module: `sheetbridge/scheduler.py`
- Thread offload helper: `sheetbridge/blocking.py` (wraps `anyio.to_thread.run_sync` for background tasks)
...
- Docker build + smoke: `.github/workflows/docker.yml`
- Compose bundle: `docker-compose.yml`
- Persistent host paths created for Compose: `data/` (`sheetbridge.db`), `tokens/` (`sheets.json`)
...
- `/bulk/append` iterates the same validation pipeline per row, drops strict key misses before hitting the cache when `UPSERT_STRICT` is true, falls back to plain inserts for missing-key rows when strict mode is disabled so `accepted` matches stored rows, preserves per-row indices in the response, and stores partial success responses under batch idempotency keys without rolling back successful inserts.
- Dead-letter rows are stored via the new `DeadLetter` SQLModel table with `reason`, original `data`, and a `created_at` timestamp to support future replay/inspection. The async retry loop now wraps `dlq_fetch`, the synchronous Google Sheets writer, and `dlq_delete` in `anyio.to_thread.run_sync` and coordinates them with an `asyncio.Semaphore` so background work stays off the main event loop.
- `RequestLogMiddleware` in `sheetbridge/logging_setup.py` emits INFO logs per request with method, path, status, and latency (ms) using standard logging formatting.
- Prometheus metrics include `sheetbridge_requests_total{method,path,status}` and `sheetbridge_latency_seconds{method,path}`. `/metrics` returns the standard text exposition format via a FastAPI router.
...
- Core dependencies now include `httpx>=0.27,<1.0` (mirroring the dev extra) so Starlette's `TestClient` and async HTTP helpers are guaranteed present during tests and local runs.
- Packaging now pins discovery to the `sheetbridge` package (`[tool.setuptools.packages.find]`) so support directories such as `docker/`, `data/`, and `tokens/` do not end up in the wheel/sdist.
...
- New/updated config knobs: `API_KEYS` (comma-separated admin API keys, default empty), `CORS_ALLOW_ORIGINS` (comma-separated origins, default `*`), `SYNC_ENABLED` (defaults to `0`), `SYNC_INTERVAL_SECONDS` (default `300`), `SYNC_JITTER_SECONDS` (default `15`), `SYNC_BACKOFF_MAX_SECONDS` (default `600`), `IDEMPOTENCY_TTL_SECONDS` (default `86400`), `LOG_LEVEL` (default `INFO`), `RATE_LIMIT_ENABLED` (default `0`), `RATE_LIMIT_RPS` (default `5.0`), `RATE_LIMIT_BURST` (default `20`), `SCHEMA_JSON_PATH` (default `schema.json`, optional), `KEY_COLUMN` (default unset/`None`), `UPSERT_STRICT` (default `1`/`True`, toggles rejection of missing keys when a key column is active), `BULK_MAX_ITEMS` (default `500`, caps `/bulk/append` payload size), `SHEETS_BATCH_SIZE` (default `200`, controls write-back chunking), `DLQ_RETRY_ENABLED` (default `1`), `DLQ_RETRY_INTERVAL` (default `300` seconds), `DLQ_RETRY_BATCH` (default `50`), and `DLQ_RETRY_CONCURRENCY` (default `4`, limits simultaneous background retries). Legacy knobs like `GOOGLE_OAUTH_CLIENT_SECRETS`, `GOOGLE_SERVICE_ACCOUNT_JSON`, `DELEGATED_SUBJECT`, `TOKEN_STORE`, and `ALLOW_WRITE_BACK` remain.
- Container/runtime knobs: `PORT` (default `8000`), `WORKERS` (default `2`, consumed by the Gunicorn entrypoint), `CACHE_DB_PATH` (set to `/data/sheetbridge.db` in Compose), and `TOKEN_STORE` (mapped to `/tokens/sheets.json`).
...
- `tests/test_metrics.py` confirms `/metrics` responds with the SheetBridge counters using a temporary cache path.
...
- `tests/test_dlq_retry.py` verifies the new `/admin/dlq/retry` endpoint returns HTTP 503 when write credentials are absent and leaves DLQ entries intact.
- `tests/test_dlq_nonblocking.py` seeds the DLQ, forces a slow Sheets writer, and asserts `/health` stays sub-200ms thanks to the non-blocking thread-pool retries.
...
- Dead-letter queue: persisted in SQLite table `deadletter` (via `sheetbridge/store.py`). Entries include `reason` (including `write_failed` for Sheets write exceptions), the original payload, and a unix `created_at` timestamp. Access via authenticated `GET /admin/dlq`; automatic retries run in the background when credentials are configured, and `POST /admin/dlq/retry` manually replays pending rows and prunes successes.
